    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            if Model_type == 0:
                ...
                p_ave = model(seqs_nanobody,seqs_antigen,device)
...
print('##########################在nai数据上测试NanoBind_pro(SabdabData){}模型：'.format(ESM2_MODEL))
...
testDataset  = seqData_NBAT_Test(seq_path='./data/Nanobody_Antigen-main/all_pair_data.seqs.fasta',pair_path = './data/Nanobody_Antigen-main/all_pair_data.pair.tsv')
test_loader = DataLoader(testDataset, batch_size=128, shuffle=False)

print(len(testDataset))

#Test
g,p1= predicting(model, device, test_loader,Model_type=1)

precision,recall,accuracy,F1_score,Top10,Top20,Top50,AUC_ROC,AUC_PR,mcc = evaluate(g,p1,thresh=0.5)
...
for threshold in threshold_range:
    _, _, _, f1, _, _, _, _, _ ,_= evaluate(g, p1, thresh=threshold)

    if f1 > best_f1:
        best_f1 = f1
        best_threshold = threshold
pe, re, a, f1, t, to, top, rc, pr,mc = evaluate(g, p1, thresh=best_threshold)

print(f"Best Threshold: {best_threshold:.2f}, Best F1 Score: {best_f1:.4f},precision:{pe:.4f},recall:{re:.4f},accuracy:{a:.4f},auroc:{rc:.4f},aupre:{pr:.4f},mcc:{mc:.4f}")
