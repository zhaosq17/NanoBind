...
class infaData_Sabdab(Dataset):
    def __init__(self, pair_path, augment = False):
        ...
        for item in data:
            ...
            #_,_,seq1,BSite2,_,seq2,_,_,_ = item
...
class seq_affinity(Dataset):
    def __init__(self, pair_path):
        super(seq_affinity,self).__init__()
        
        data = pd.read_csv(pair_path).values.tolist()
        self.seq_data = list()
        for item in data:
            ID1,ab1,ag1,ID2,ab2,ag2,label = item

            self.seq_data.append([ab1,ag1,ab2,ag2,label])
    ...
    def __getitem__(self,i):
        ab1,ag1,ab2,ag2,label = self.seq_data[i]

        return ab1,ag1,ab2,ag2,label


class anli_aff(Dataset):
    def __init__(self, pair_path):
        super(anli_aff,self).__init__()
        ...
        for i in data:
            ...
            self.seq_data.append([ab,ag,aff])
