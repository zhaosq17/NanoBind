## Introduction
NanoBind is a unified framework for comprehensive prediction of nanobody-antigen molecular recognition, specifically comprising five sub-models. Among them, NanoBind-seq and NanoBind-pro predict whether binding occurs, 
NanoBind-site predicts antigen-binding interface residues, NanoBind-pair predicts the relative affinity strength between two nanobody-antigen complexes, and NanoBind-affi assesses the affinity range of a nanobody-antigen complex.
...
1. **`data`**: This folder contains all the datasets used for binding prediction, antigen-binding interface residue prediction, and affinity assessment, and they have been split into training, validation, and test sets.
...
10. **'predict_site.py'**: Performing rapid antigen binding interface residues prediction using the NanoBind-affi model.
11. **'test_seq.py'**: For reproducing the binding prediction results using NanoBind-seq as presented in the NanoBind paper.
12. **'test_site.py'**: For reproducing the antigen binding interface residues prediction results using NanoBind-site as presented in the NanoBind paper.
...
Simple run each line of code below in sequence to obtain the results presented in the paper: NanoBind binding predictions, antigen-binding interface residue predictions, relative affinity strength predictions, and affinity range predictions:
...
### Applying the MethFinder framework for custom training
